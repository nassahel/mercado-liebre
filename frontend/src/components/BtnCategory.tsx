...
const BtnCategory = (props: Props) => {
    ...
    return (
        <div className='relative'>
            <Link href="" onMouseEnter={fetchCategories} onMouseLeave={() => setModal(false)} className='flex items-center gap-1'><span>Categorías</span> <div className='pt-1'><IoIosArrowDown className='text-xs text-neutral-500' /></div>  </Link>
            {
                modal && <div onMouseEnter={() => setModal(true)} onMouseLeave={() => setModal(false)} className='cursor-pointer z-20 top-0 left-0 absolute pt-5'>
                    <div className='ms-[4.3rem] trinagle'></div>
                    <div className=' bg-zinc-800  text-white  rounded-md py-5 '>
                        <ul>
                            {
                                categories.map((item: any, i) => (
                                    <li key={i} className='w-[15rem] h-8 flex items-center  hover:bg-blue-600 cursor-pointer'>
